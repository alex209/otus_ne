...
Root портом на коммутаторе **S2** выбран порт *e0/0* а на коммутаторе **S3** выбран порт *e0/2*.

Коммутатор **S2** подключен к корневому коммутатору портами *e0/0* и *e0/1* оба порта имеют одинаковую стоимость и одинаковый приоритет поэтому root портом назначается порт *e0/0* т.к. он имеет наименьший номер.

Коммутатор **S3** подключен к корневому коммутатору портами *e0/2* и *e0/3* оба порта имеют одинаковую стоимость и одинаковый приоритет поэтому root портом назначается порт *e0/2* т.к. он имеет наименьший номер.

### Вопросы для повторения
+ Q. Какое значение протокол STP использует первым после выбора корневого моста, чтобы определить выбор порта?

    A. стоимость пути до корневого коммутатора
